...
print("#######################################################")
try:
    connector = pymysql.connect(host=bootstrap.host, user=bootstrap.user, password=bootstrap.password)
    cursor = connector.cursor()
    print("Connection to Wamp : Successfull") 
except:
    print("Error, could not connect to the SQL database. Please start WampServer again.")
    os.system("pause")
    sys.exit()
...
try:
    connector = pymysql.connect(host=bootstrap.host, user=bootstrap.user, password=bootstrap.password, db=database_name)
    cursor = connector.cursor()
...
print("JDBC connection string : jdbc:mysql://localhost/{0}?user={1}&password=[EnterYourPasswordHere]".format(database_name, bootstrap.user))
...
group_names = []
for group in present_groups:
    if group[0] not in group_names:
        group_names.append(group[0])
    else :
        present_groups.remove(group)
group_names = []
for group in present_groups:
    if group[0] not in group_names:
        group_names.append(group[0])
